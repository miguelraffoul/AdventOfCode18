...
int getCombatOutcome( void );
pair<int, string> playTurn( pair<int, string> );
...
void attackEnemy( const string&, const char& );
int getTotalRemainingHitPoints( void );

int main( void ) {
    readInput();
    cout << getCombatOutcome() << endl;
    return EXIT_SUCCESS;
}
...
int getCombatOutcome( void ) {
    ...
    while( elfs.size() > 0 && goblins.size() > 0 ) {
        ...
        while( player_order.size() > 0 ) {
            tmp_player_order.insert( playTurn( *(player_order.begin()) ) );
...
pair<int, string> playTurn( pair<int, string> player_details ) {
    ...
    attack_direction = isInAttackRange( player_details.second );
    if( attack_direction != '-' ) {
        attackEnemy( player_details.second, attack_direction );
    }
...
void attackEnemy( const string& player_id, const char& attack_direction ) {
